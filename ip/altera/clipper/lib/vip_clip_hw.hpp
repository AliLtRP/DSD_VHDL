    ...
    void controlMonitor()
    {
        ...
        for (;;)
        {
            ...
            if (isRead)
            {
                ...
                control->returnReadData(sc_int<CTRL_INTERFACE_WIDTH>(running));
            }
            else
            {
                sc_uint<CTRL_INTERFACE_WIDTH> thisRead = sc_uint<CTRL_INTERFACE_WIDTH>(control->getWriteData());
