...
#------------------------------------------------------------------------------------------------------------------------------
set locked_reg_collection [get_keepers -nowarn -nocase *a2b*|*u_dual_link_sync|current_state*]
foreach_in_collection keeper $locked_reg_collection {
   set locked_reg_keeper_name [get_object_info -name $keeper]
   set_false_path -from [get_keepers -nocase {*u_receive|*u_format|trs_locked}] -to [get_keepers -nocase $locked_reg_keeper_name]
   set_false_path -from [get_keepers -nocase {*u_receive|*u_align|align_locked}] -to [get_keepers -nocase $locked_reg_keeper_name]
   set_false_path -from [get_keepers -nocase {*u_dual_link_sync|*stream_a_*_fifo|*|wrfull_eq_comp_*sb_mux_reg}] -to [get_keepers -nocase $locked_reg_keeper_name]
   ...
   set_false_path -from [get_keepers -nocase {*u_receive|*u_align|align_locked}] -through [get_pins -nocase -compatibility_mode {*u_dual_link_sync|*stream_*_fifo|*rd*}]
   ...
   set_false_path -from [get_keepers -nocase {*u_dual_link_sync|*_fifo_wrfull_reg[*]}] -to [get_keepers -nocase {*u_dual_link_sync|*stream_a_*_fifo|*}]
	set_false_path -to   [get_keepers -nocase {*u_dual_link_sync|vpid_fifo_a2b.vpid_line_f*_sync[*]}]
   set_false_path -to   [get_keepers -nocase {*u_dual_link_sync|vpid_fifo_a2b.vpid_valid_*_3gb}]
}
