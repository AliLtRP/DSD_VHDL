...
cp -r <IP root directory>\altera\altera_dp\sim_example\av <working directory>

After copying, your working directory contains the following files:

System Verilog HDL Design Files:
* cv_dp_harness.sv			Top-level test harness

Verilog HDL design files:
* cv_dp_example.v			Design under test (DUT)
...
* cv_dp.v				DisplayPort IP core
* cv_xcvr_reconfig.v			Transceiver reconfiguration core
...
* Merge the two resulting msim_setup.tcl scripts to create a single mentor/msim_setup.tcl:
  ip-make-simscript --spd=./cv_xcvr_reconfig.spd --spd=./cv_dp.spd
...
to use the HBR (2.7 G) rate. A successful result is seen by the CTS test automation logic's CRC checks.
