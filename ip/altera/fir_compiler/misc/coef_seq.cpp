...
//	int coef_width;
	...
	char in_name[BUFFER_SIZE];
	char *non;

	char temp_char[TEMP_CHAR_SIZE];
...
/*********************************************************************************************/
	...
	if (argc >= 11 ) // if the user calls this program with no arguments ...
	{
		...
		strncpy(in_name,argv[1],BUFFER_SIZE);// input file name, full path
		strncpy(out_name,argv[2],BUFFER_SIZE);// out file name, full path

		strncpy(temp_char, argv[3], TEMP_CHAR_SIZE);//structure type
		...
		strncpy(temp_char, argv[4], TEMP_CHAR_SIZE);//coef_storage type
		...
		strncpy(temp_char, argv[5] , TEMP_CHAR_SIZE);//NOSYM
		...
		strncpy(temp_char, argv[8], TEMP_CHAR_SIZE);//poly_type
		poly_type = SGL;
		...
		if(struct_type == MCV)
		{
			...
			else
			{
				...
				for(i=0;i<poly_fact;i++)
				{
					...
					if(mcv_reload_zero_insert > 0)
					{
						memset(tmp_coef, 0, 2048 * sizeof(int)); // I suspect this preinitialisation is not required but it should please coverity - DRE
...
//		temp_file.open("c:/cvs/sj/projects/fir_compiler/work/gui/temp/coef_arr.txt", ios::out);
		...
		if (i!=0)
		{
			if(struct_type == MCV)
			{
				if (num_cycles == 2 && coef_store_type == LC)
				{
					// re-order the coefficients even to odd and vice versa.
					out_file <<  coef[1] ;
...
{
        int j, k;
        const unsigned int last_bit = 0x1;

        // fill in the answer, backwards
        j = 0;
        do
        {
                if (i & last_bit)
        ...
        j--;
        // Now j contains number of valid characters in str_deci[]

        // Form the right answer by reversing the accumulated characters
        for (k = 0; k <= j/2; k++)
        {
                char temp = str_deci[k];
                str_deci[k] = str_deci[j - k];
                str_deci[j - k] = temp;
        }
        str_deci[j+1] = '\0';
}
