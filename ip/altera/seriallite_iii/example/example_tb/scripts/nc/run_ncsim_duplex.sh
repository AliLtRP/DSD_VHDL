...
irun -compile -sv +define+DUPLEX_MODE +define+SIMULATION +incdir+../../src+../ -F seriallite_src_lst_ncsim.txt

# Run the simulation

ncelab -access +w+r+c -defparam test_env.lanes=4 test_env
ncsim -run +random_seed=123456 +define+SIMULATION +define+DUPLEX_MODE +test_name=data_forwarding test_env
