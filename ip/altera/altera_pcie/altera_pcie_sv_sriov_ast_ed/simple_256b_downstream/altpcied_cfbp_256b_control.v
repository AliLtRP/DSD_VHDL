...
#(
  ...
  //input          RxStEmpty_i,
   ...
   output wire                            mbox_sel_o,                 // Select mailbox registers
    ...
    //============================
      ...
      output [7:0]    rxm_bar_hit_tlp0_o, 
      output [7:0]    rxm_bar_hit_fn_tlp0_o, 
 ...
 reg               rx_pending_reg;
 reg  [7:0]        rxm_bar_hit_tlp0; 
 reg  [7:0]        rxm_bar_hit_fn_tlp0; 
...
always @(posedge Clk_i or negedge Rstn_i) begin 
    if(~Rstn_i)
      ...
      header_poll_counter <= header_poll_counter + 4'h1;
end
...
always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i) begin
      ...
      //f1_hit_reg          <= 1'b0;
      pf0_bar0_hit        <= 1'b0;
      rxm_bar_hit_tlp0    <= 8'h0;
      rxm_bar_hit_fn_tlp0 <= 8'h0;
    end else if(ld_header) begin
      rx_header1_reg    <= RxStData_i[ 63: 0];
      rx_header2_reg    <= RxStData_i[127:64];
      rx_bar_hit_reg    <= |(rx_st_bar_hit_tlp0[7:0]);
      //f1_hit_reg        <= |(rx_st_bar_hit_tlp0)    & (rx_st_bar_hit_fn_tlp0 != 8'h0); //Map any BAR of any function that is not PF0 to the same memory location
      pf0_bar0_hit      <=  (rx_st_bar_hit_tlp0[0]) & (rx_st_bar_hit_fn_tlp0 == 0) ;
      rxm_bar_hit_tlp0    <= rx_st_bar_hit_tlp0;
      rxm_bar_hit_fn_tlp0 <= rx_st_bar_hit_fn_tlp0;
    end       
end    
...
always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i)
      ...
      rx_4qword_counter <= adjusted_data_4qw - 8'b1; 
    else if(RxStReady_o)
      rx_4qword_counter <= rx_4qword_counter - 8'b1;
  end   
...
// Bar0 Avalon-MM request
 ...
 assign mbox_sel_o       =  pf0_bar0_hit & avl_addr_reg[10];
...
/// Tie off the inputs when not available
 ...
 assign rxm_bar_hit_tlp0_o    = rxm_bar_hit_tlp0; 
 assign rxm_bar_hit_fn_tlp0_o = rxm_bar_hit_fn_tlp0; 
...
always @*  // only first completion uses the fbe for byte count
 begin
  case({rx_fbe, rx_lbe})
    8'b1000_0001 : abort_byte_count = rx_byte_len - 12'h6;
    8'b1000_0011 : abort_byte_count = rx_byte_len - 12'h5;
    8'b1000_1111 : abort_byte_count = rx_byte_len - 12'h3;
    
    8'b1100_0001 : abort_byte_count = rx_byte_len - 12'h5;
    8'b1100_0011 : abort_byte_count = rx_byte_len - 12'h4;
    8'b1100_1111 : abort_byte_count = rx_byte_len - 12'h2;
    
    8'b1111_0001 : abort_byte_count = rx_byte_len - 12'h3;
    8'b1111_0011 : abort_byte_count = rx_byte_len - 12'h2;
...
//assign ep_bus_dev = {bus_num_f0, device_num_f0};
assign ep_bus_dev = {bus_num_f0, rxm_bar_hit_fn_tlp0[7:3]};
...
assign  cpl_funcno =   is_invalid_cfg0  ? 3'h0  : (is_cfg0 ? rx_funcno_reg : rxm_bar_hit_fn_tlp0[2:0]);
assign  cpl_header = {                                            
