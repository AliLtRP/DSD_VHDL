...
void 
io_write(alt_u32 *addr, alt_u32 data)
{
	//printf("\tSEQ: Writing 0x%x to %p\n", data, addr);
...
alt_u32
io_read(alt_u32 *addr)
{
	alt_u32 val;
	
	//printf("\tSEQ: Reading from %p\n", addr);
...
{
	// printf("\t%lld: BFM: get_request called\n", sim_time);
...
{
	//printf("\t%lld: BFM: send_response called: is_write=%c addr=%x data=%x\n",
	//     sim_time, is_write ? 'Y' : 'N', *addr, *data);
