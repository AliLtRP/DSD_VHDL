...
sub usage
{
    ...
    print "\n";
    print "Creates two output files using CPU perl modules as reference:\n";
...
create_file
{
    ...
    nevada_isa::convert_to_c($nevada_isa_info, $c_lines, $h_lines);

    my $control_reg_args = 
      nevada_control_regs::create_control_reg_args_default_configuration();
    my $control_reg_info = 
      nevada_control_regs::validate_and_elaborate($control_reg_args);
    nevada_control_regs::convert_to_c($control_reg_info, $c_lines, $h_lines);

    my $nevada_testbench_info = 
      nevada_testbench::validate_and_elaborate($control_reg_info);
    nevada_testbench::convert_to_c($nevada_testbench_info, $c_lines, $h_lines);

    my $inst_args = nevada_insts::create_inst_args_gen_isa_configuration();
    my $inst_info = nevada_insts::validate_and_elaborate($inst_args);
    nevada_insts::convert_to_c($inst_info, $c_lines, $h_lines);

    my $mmu_args = nevada_mmu::create_mmu_args_default_configuration();
    my $elaborated_mmu_info = nevada_mmu::validate_and_elaborate($mmu_args);
    nevada_mmu::convert_to_c($elaborated_mmu_info, $c_lines, $h_lines);

    my $elaborated_exception_info = nevada_exceptions::validate_and_elaborate();
    nevada_exceptions::convert_to_c($elaborated_exception_info,
      $c_lines, $h_lines);
...
{
    print HFD "/*\n";
    print HFD " * This file defines Nevada instruction set constants.\n";
    ...
    print HFD "\n#ifndef _MIPS32_ISA_H_\n#define _MIPS32_ISA_H_\n";
}

sub
add_h_footer
{
    print HFD "\n#endif /* _MIPS32_ISA_H_ */\n";
}
