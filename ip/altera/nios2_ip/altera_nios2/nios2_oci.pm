...
make_nios2_oci
{
  ...
  if ($Opt->{oci_offchip_trace}) {
    ...
    e_assign->adds (
      { lhs => "tr_clk",  rhs => 0, tag => "simulation", },
  ...
  $top_module->add_contents (
    ...
    e_assign->news (
      ...
      ["jtag_debug_module_reset","~reset_n"],
    ),
  );
  

  if ($Opt->{avalon_debug_port_present}) {
       print "\nINFO: jtag_debug_module is creating an avalon port " .
