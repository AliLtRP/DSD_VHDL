...
gen_controls
{
    ...
    my $ioregion_dcache = 0;
    if (defined $Opt->{ioregion_dcache}) {
        $ioregion_dcache = manditory_bool($Opt, "ioregion_dcache");
    }
            ...
            e_assign->adds(
              [["M_mem_bypass_non_io", 1], 
                "M_alu_result[$datapath_msb] | 
                  (~M_dmpu_c & ${cs}_config_reg_pe & ~W_debug_mode)"],
            );
        } else {
