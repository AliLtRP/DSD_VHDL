...
create_clock -name {tx_mac_clk_clk} -period 3.35 -waveform { 0.000 1.675 } [get_ports {*tx_mac_clk_clk}]
create_clock -name {rx_mac_clk_clk} -period 3.35 -waveform { 0.000 1.675 } [get_ports {*rx_mac_clk_clk }]
...
if {[catch {get_port_info -is_input_port [get_ports -nowarn {*rx_oob_in_sys_clk}]} errmsg ]} {
    ...
    create_clock -name {rx_oob_in_sys_clk} -period 3.35 -waveform { 0.000 1.675 } [get_ports {*rx_oob_in_sys_clk}]
    #100 MHz 
