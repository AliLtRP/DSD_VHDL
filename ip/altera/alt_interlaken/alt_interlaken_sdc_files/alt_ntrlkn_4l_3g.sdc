...
create_clock -name {tx_mac_clk_clk} -period 10.240 -waveform { 0.000 5.12 } [get_ports {*tx_mac_clk_clk}]
create_clock -name {rx_mac_clk_clk} -period 10.240 -waveform { 0.000 5.12 } [get_ports {*rx_mac_clk_clk }]
...
if {[catch {get_port_info -is_input_port [get_ports -nowarn {*rx_oob_in_sys_clk}]} errmsg ]} {
    ...
    create_clock -name {rx_oob_in_sys_clk} -period 10.240 -waveform { 0.000 5.12 } [get_ports {*rx_oob_in_sys_clk}]
    #100 MHz 
...
if {[catch {get_port_info -is_input_port [get_ports -nowarn {*tx_lane_clk_clk}]} errmsg ]} {
    ...
    create_clock -name {tx_lane_clk_clk} -period 6.400 -waveform { 0.000 3.200 } [get_ports {*tx_lane_clk_clk}]   
    create_clock -name {rx_lane_clk} -period 6.400 -waveform { 0.000 3.200 } [get_ports {*rx_lane_clk*_export*}]
