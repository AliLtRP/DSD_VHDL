...
create_clock -name {tx_mac_clk_clk} -period 3.88 -waveform { 0.000 1.94 } [get_ports {*tx_mac_clk_clk}]
create_clock -name {rx_mac_clk_clk} -period 3.88 -waveform { 0.000 1.94 } [get_ports {*rx_mac_clk_clk }]

if {[catch {get_port_info -is_input_port [get_ports -nowarn {*rx_oob_in_sys_clk}]} errmsg ]} {

    
    puts "Info:Out-of-band flow control is not included"


} else {

    create_clock -name {rx_oob_in_sys_clk} -period 3.88 -waveform { 0.000 1.94 } [get_ports {*rx_oob_in_sys_clk}]
    #100 MHz 
...
if {[catch {get_port_info -is_input_port [get_ports -nowarn {*cal_blk_clk}]} errmsg ]} {
    puts "Info: Transceiver is not included in this design"
...
if {[catch {get_port_info -is_input_port [get_ports -nowarn {*tx_lane_clk_clk}]} errmsg ]} {
    ...
    puts "Info: Transceiver is included in this design"

} else {
    create_clock -name {tx_lane_clk_clk} -period 3.88 -waveform { 0.000 1.94 } [get_ports {*tx_lane_clk_clk}]
    create_clock -name {rx_lane_clk} -period 3.88 -waveform { 0.000 1.94 } [get_ports {*rx_lane_clk*_export*}]
