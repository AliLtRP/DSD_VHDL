...
// Total XOR inputs 302
//
// Special signal relations -
//    none
//

module alt_ntrlkn_12l_10g_crc24_zer64_flat (c,crc_out);
input[23:0] c;
output[23:0] crc_out;
wire[23:0] crc_out;
...
assign crc_out[14] =
    ...
    c[17] ^ c[18] ^ c[19] ^ c[21] ^ c[22] ^ c[23];

assign crc_out[15] =
    c[1] ^ c[2] ^ c[3] ^ c[4] ^ c[8] ^ c[9] ^
    c[10] ^ c[11] ^ c[12] ^ c[13] ^ c[16];

assign crc_out[16] =
    c[2] ^ c[3] ^ c[4] ^ c[5] ^ c[9] ^ c[10] ^
    c[11] ^ c[12] ^ c[13] ^ c[14] ^ c[17];
