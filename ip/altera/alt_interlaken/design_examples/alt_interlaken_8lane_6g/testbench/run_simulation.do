...
if { [file exists $env(QUARTUS_ROOTDIR)/eda/sim_lib] } {
  ...
  vlog -timescale "1 ps / 1 ps" +acc ../alt_interlaken_8lane_6g/simulation/*.v
  vlog -timescale "1 ps / 1 ps" +acc -sv *.sv
  vlog -timescale "1 ps / 1 ps" +incdir+../alt_interlaken_8lane_6g/simulation/submodules/ +acc -sv ../alt_interlaken_8lane_6g/simulation/submodules/*.v
  vlog -timescale "1 ps / 1 ps" +acc -sv ../alt_interlaken_8lane_6g/simulation/submodules/mentor/*.v
} else {
...
if [regexp {ModelSim ALTERA} [vsim -version]] {
	vsim -L lpm_ver -L altera_mf_ver -L sgate_ver -L stratixiv_hssi_ver  -t 1ps alt_interlaken_8lane_6g_tb 
} else {
	vsim -t 1ps alt_interlaken_8lane_6g_tb
}
add wave *
add wave alt_interlaken_8lane_6g_tb/dut/interlaken_0/*
run -all
quit
