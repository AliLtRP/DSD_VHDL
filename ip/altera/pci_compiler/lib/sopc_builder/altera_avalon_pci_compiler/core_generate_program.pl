...
if($to_skip == 0){
	$result = system("\"$megafunction_path/../../common/ip_toolbench/v$iptb_version/bin/ip_toolbench.exe\" -wizard_file:\"$megafunction_path/ip_toolbench/$wizard_jar\" -flow_dir:\"$megafunction_path/../../common/ip_toolbench/v$iptb_version/bin\" -sopc -silent $iptb_sopc_args");
